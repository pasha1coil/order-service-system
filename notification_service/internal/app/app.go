...
func Run(ctx context.Context, config initialize.Config, logger *zap.Logger) error {
	...
	logger.Info("service start on <Run> of <app>", zap.String("service", "notification"), zap.Any("config", config))
	...
	logger.Info("service stopped on <Run> of <app>", zap.String("service", "notification"))
	return nil
}
